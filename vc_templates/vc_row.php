...
if ( $particles_effect != 'none' ) {
	...
	if ( $particles_effect != 'custom' ) {
		...
		$wrapper_attributes[] = 'data-partcles-effect="'.esc_url($config_url).'"';
	}else{
		$css_classes[] = 'vc_particles-effect';
		$particles_json = str_replace("<br />", " ", $particles_json );
		$wrapper_attributes[] = 'data-partcles-json="'.esc_attr($particles_json).'"';
	}
}
...
echo apply_filters('bt_vc_row_filter', $output);

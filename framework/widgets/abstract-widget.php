 ...
 */
abstract class bears_Widget extends WP_Widget {
			...
			switch ( $setting['type'] ) {
				...
				case "select" :
					...
					$data_script = 'jQuery(document).ready(function($) {
										$( ".'.esc_attr($key.$date_sl) .' select" ).change(function() {
											var val_op, val_el;
											var $_this = jQuery(this);
											val_op = $_this.val();
											$( \'[data-element="'.esc_attr($key).'"]\' ).each(function() {
				...
				case "checkbox" :
					...
					$data_script = 'jQuery(document).ready(function($) {
										$( ".'.esc_attr($key.$date_cb).' input" ).change(function() {
											var val_op, val_el;
											val_op = $( ".'.esc_attr($key.$date_cb).' input:checked" ).val();                                                        
											$( \'[data-element="'.esc_attr($key).'"]\' ).each(function() {
												val_el = $(this).attr("data-value").split(",");
												for(var i=0;i<val_el.length;i++){
													if(val_op == val_el[i]){
				...
				case "bears_taxonomy" :
					...
					?>
					<div class="bears_taxonomy <?php echo esc_attr( $key ); ?>" <?php echo esc_attr( $attr_hidden ); ?>>
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>

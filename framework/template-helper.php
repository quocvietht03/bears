...
if ( ! function_exists( 'bears_setup' ) ) {
	function bears_setup() {
		...
		register_nav_menus( array(
			'primary_menu'   => esc_html__( 'Primary Menu','bears' ),
...
if (!function_exists('bears_logo')) {
	function bears_logo($url = '', $height = 30) {
		if(!$url){
			$url = get_template_directory_uri().'/assets/images/logo.png';
		}
		echo '<a href="'.home_url('/').'"><img class="logo" style="height: '.esc_attr($height).'px; width: auto;" src="'.esc_url($url).'" alt="'.esc_attr__('Logo', 'bears').'"/></a>';
	}
}
...
if (!function_exists('bears_page_breadcrumb')) {
  function bears_page_breadcrumb($home_text = 'Home', $delimiter = '-') {
		...
		if(is_front_page()){
			echo '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Front Page', 'bears');
		}elseif(is_home()){
			echo  '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Blog', 'bears');
		}else{
		...
		if ( get_query_var('paged') ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
				echo ' <span class="bt-deli">' . $delimiter . '</span> ' . esc_html__('Page', 'bears') . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
  }
}
...
if ( ! function_exists( 'bears_post_nav' ) ) {
	function bears_post_nav() {
		...
		<nav class="bt-post-nav clearfix">
			<?php
				previous_post_link('<div class="bt-post-nav--item bt-prev"><span>' . esc_html__('Previous Post', 'bears') . '</span><h3>%link</h3></div>');
				next_post_link('<div class="bt-post-nav--item bt-next"><span>' . esc_html__('Next Post', 'bears') . '</span><h3>%link</h3></div>');
			?>
		</nav>
		<?php
	}
...
if ( ! function_exists( 'bears_paginate_links' ) ) {
	function bears_paginate_links($wp_query) {

<?php
/* Count post view. */
if ( ! function_exists( 'bears_set_count_view' ) ) {
  function bears_set_count_view(){
    $post_id = get_the_ID();

  	if(is_single() && !empty($post_id) && !isset($_COOKIE['bears_post_view_'. $post_id])){
      ...
      /* set cookie. */
      setcookie('bears_post_view_'. $post_id, $post_id, time() * 20, '/');
    }
  }
}
add_action( 'wp', 'bears_set_count_view' );

/* Post count view */
if ( ! function_exists( 'bears_get_count_view' ) ) {
  function bears_get_count_view() {
  	$post_id = get_the_ID();
      $views = get_post_meta($post_id , '_post_count_views', true);

      $views = $views ? $views : 0 ;
      $label = $views > 1 ? esc_html__('Views', 'bears') : esc_html__('View', 'bears') ;
      return $views . ' ' . $label;
  }
}

/* Post Reading */
if ( ! function_exists( 'bears_reading_time_render' ) ) {
  function bears_reading_time_render() {
...
/* Single Post Title */
if ( ! function_exists( 'bears_single_post_title_render' ) ) {
	function bears_single_post_title_render() {
		ob_start();
    ?>
...
/* Post Title */
if ( ! function_exists( 'bears_post_title_render' ) ) {
	function bears_post_title_render() {
		ob_start();
    ?>
      <h3 class="bt-post--title">
...
/* Post Featured */
if ( ! function_exists( 'bears_post_featured_render' ) ) {
	function bears_post_featured_render($image_size = 'full') {
		ob_start();
...
/* Post Cover Featured */
if ( ! function_exists( 'bears_post_cover_featured_render' ) ) {
	function bears_post_cover_featured_render($image_size = 'full') {
		ob_start();
    ?>
    <div class="bt-post--featured">
      <a href="<?php the_permalink(); ?>">
...
/* Post Publish */
if ( ! function_exists( 'bears_post_publish_render' ) ) {
	function bears_post_publish_render() {
		ob_start();
...
/* Post Short Meta */
if ( ! function_exists( 'bears_post_short_meta_render' ) ) {
	function bears_post_short_meta_render() {
		ob_start();
      ...
      <div class="bt-post--meta">
        <?php
          ...
          echo bears_reading_time_render();
        ?>
      </div>
		<?php
...
/* Post Meta */
if ( ! function_exists( 'bears_post_meta_render' ) ) {
	function bears_post_meta_render() {
		ob_start();
        ...
        <li class="bt-meta bt-meta--author">
          ...
          <a href="<?php comments_link(); ?>">
            ...
            <?php comments_number( esc_html__('0 Comments', 'bears'), esc_html__('1 Comment', 'bears'), esc_html__('% Comments', 'bears') ); ?>
          </a>
        </li>
			</ul>
		<?php
...
/* Post Content */
if ( ! function_exists( 'bears_post_content_render' ) ) {
	function bears_post_content_render() {
...
/* Post tag */
if ( ! function_exists( 'bears_tags_render' ) ) {
	function bears_tags_render() {
		ob_start();
		if(has_tag()){
			?>
        ...
        <?php
          if(has_tag()){
            the_tags( '<span>'.esc_html__('Tags: ', 'bears').'</span>', '', '' );
          }
        ?>
      </div>
			<?php
...
/* Post share */
if ( ! function_exists( 'bears_share_render' ) ) {
	function bears_share_render() {

		$social_item = array();
		$social_item[] = '<li>
                        <a target="_blank" data-btIcon="fa fa-facebook" data-toggle="tooltip" title="'.esc_attr__('Facebook', 'bears').'" href="https://www.facebook.com/sharer/sharer.php?u='.get_the_permalink().'">
		...
		$social_item[] = '<li>
                        <a target="_blank" data-btIcon="fa fa-twitter" data-toggle="tooltip" title="'.esc_attr__('Twitter', 'bears').'" href="https://twitter.com/share?url='.get_the_permalink().'">
                          <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
		...
		$social_item[] = '<li>
                        <a target="_blank" data-btIcon="fa fa-google-plus" data-toggle="tooltip" title="'.esc_attr__('Google Plus', 'bears').'" href="https://plus.google.com/share?url='.get_the_permalink().'">
		...
		$social_item[] = '<li>
                        <a target="_blank" data-btIcon="fa fa-linkedin" data-toggle="tooltip" title="'.esc_attr__('Linkedin', 'bears').'" href="https://www.linkedin.com/shareArticle?url='.get_the_permalink().'">
                          <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
		...
		$social_item[] = '<li>
                        <a target="_blank" data-btIcon="fa fa-pinterest" data-toggle="tooltip" title="'.esc_attr__('Pinterest', 'bears').'" href="https://pinterest.com/pin/create/button/?url='.get_the_permalink().'">
		...
		if(has_tag()){
          ...
          <?php
            if(!empty($social_item)){
              echo '<span>'.esc_html__('Share: ', 'bears').'</span><ul>'.implode(' ', $social_item).'</ul>';
            }
          ?>
				</div>
			<?php
...
/* Author */
if ( ! function_exists( 'bears_author_render' ) ) {
	function bears_author_render() {
...
/* Related posts */
if ( ! function_exists( 'bears_related_posts' ) ) {
	function bears_related_posts() {
    $post_id = get_the_ID();
    $cat_ids = array();
    ...
    if($list_posts->have_posts()) {
        ...
        <?php esc_html_e('Related Posts', 'bears'); ?>
      </h2>
...
//Comment Field Order
function bears_comment_fields_custom_order( $fields ) {
...
add_filter( 'comment_form_fields', 'bears_comment_fields_custom_order' );

/* Custom comment list */
if ( ! function_exists( 'bears_custom_comment' ) ) {
  function bears_custom_comment($comment, $args, $depth) {
        ...
        ?>
          ...
          <?php echo get_comment_date(); ?>
  			...
  			<?php if ( $comment->comment_approved == '0' ) : ?>
  				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'bears' ); ?></em>
  			<?php endif; ?>
        <div class="bt-text">
    			<?php comment_text(); ?>
        </div>

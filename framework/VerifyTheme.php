  ...
  class BearsthemesCommunicator{
    ...
    function registerDomain($purchaseCode, $domain) {
        ...
        if ( is_wp_error( $response ) ) {
           ...
           $this->message = __("Something went wrong on unregister your domain: ", "bears").$error_message;
           return null;
        } else {
           return true;
        }
    ...
    function getConnectedDomains($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get connected domain with your purchase code: ", "bears").$error_message;
         return null;
      } else {
    ...
    function getPurchaseInformation($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get purchase information: ", "bears").$error_message;
         return null;
      } else {
         $result = json_decode($response['body']);
         return $result;
...
class VerifyTheme {
    ...
    // add style admin
      ...
      if(!$this->isInstallationLegit){
        $setting_page = admin_url('options-general.php?page=verifytheme_settings');
        $message = esc_html__( "Important notice: In order to receive all benefits of our theme, you need to activate your copy of the theme. \nBy activating the theme license you will unlock premium options - import demo data, install, update plugins and official support. Please visit Envato Settings page to activate your copy of the theme", 'bears' );
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if ($ok_purchase_code) {
          ...
          if($communicator->message){
            ...
            $message .= __("Invalid purchase code<br />","bears");
          }
        }

 ...
 */
define("ITEM_ID","22165017"); // Replace it by current item id
...
class BearsthemesAPI {
    ...
    function request($endpoint, $headers=array(), $curlParams=array()) {
        ...
        try {
            ...
            if ($curl_errno > 0) {
                $message = sprintf("%s (%s): %s \n", __('cURL Error', 'bears'), $curl_errno, $curl_error);
                return json_encode(array( 'error' => $message ) );
            }
        } catch (Exception $e) {
            return null;
...
class VerifyTheme {
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if ($ok_purchase_code) {
            ...
            $message .=  wp_kses( __( 'Invalid purchase code<br />', 'bears' ), array( 'br' => array() ) );
        }

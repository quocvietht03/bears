...
function bears_mime_types($mimes) {
  $mimes['svg'] = 'image/svg+xml';
  return $mimes;
}
add_filter('upload_mimes', 'bears_mime_types');
...
if (!function_exists('bears_enqueue_scripts')) {
	function bears_enqueue_scripts() {
		...
		wp_enqueue_style( 'bears-main', get_template_directory_uri().'/assets/css/main.css',  array(), false );
...
if (!function_exists('bears_enqueue_admin_scripts')) {
	function bears_enqueue_admin_scripts(){
		wp_enqueue_script( 'bears-admin-main', get_template_directory_uri().'/assets/js/admin-main.js', array('jquery'), '', true);
		wp_enqueue_style( 'bears-admin-main', get_template_directory_uri().'/assets/css/admin-main.css', array(), false );
	}
...
if(function_exists('get_field')){
	/* Orbit circle effect */
	function bears_body_class($classes) {
	...
	add_filter('body_class', 'bears_body_class');

	/* Search filter results */
	function bears_search_filter_results($query) {
	...
	if($team_sft || is_tax('team_categories')) {
		add_action('pre_get_posts','bears_search_filter_results');
	}
}
